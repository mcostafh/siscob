object frm_RelDemonst: Tfrm_RelDemonst
  Left = 322
  Top = 106
  Width = 478
  Height = 420
  Caption = 'Demonstrativo de Cobran'#231'a'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIForm
  KeyPreview = True
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object lb_mens: TLabel
    Left = 73
    Top = 348
    Width = 13
    Height = 13
    Caption = '...'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    Visible = False
  end
  object Label3: TLabel
    Left = 7
    Top = 104
    Width = 91
    Height = 13
    Caption = 'Carteira - Cobrador:'
  end
  object Label1: TLabel
    Left = 8
    Top = 62
    Width = 32
    Height = 13
    Caption = 'Cliente'
  end
  object Gauge_atualizando: TGauge
    Left = 96
    Top = 331
    Width = 233
    Height = 16
    Progress = 0
    Visible = False
  end
  object Label16: TLabel
    Left = 10
    Top = 249
    Width = 67
    Height = 13
    Caption = 'Tipo do T'#237'tulo'
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 470
    Height = 41
    Align = alTop
    BevelOuter = bvLowered
    Caption = 'Demonstrativo de Cobran'#231'a'
    Color = 4210688
    Font.Charset = ANSI_CHARSET
    Font.Color = clWhite
    Font.Height = -19
    Font.Name = 'Arial Black'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    object Image1: TImage
      Left = 1
      Top = 1
      Width = 468
      Height = 39
      Align = alClient
      Picture.Data = {
        
        8A28A0028A28A0028A28A0028A28A0028A28A00FFFD9}
      Stretch = True
      Transparent = True
    end
    object Label14: TLabel
      Left = 75
      Top = 6
      Width = 292
      Height = 27
      Caption = 'Demonstrativo de Cobran'#231'a'
      Color = clBlack
      ParentColor = False
    end
  end
  object Panel2: TPanel
    Left = 8
    Top = 279
    Width = 401
    Height = 47
    BevelOuter = bvNone
    TabOrder = 1
    object btn_imprimir: TSpeedButton
      Left = 46
      Top = 2
      Width = 108
      Height = 41
      AllowAllUp = True
      Caption = '&Imprimir'
      Flat = True
      Glyph.Data = {
        
        F0DDDDDDDDD7FFFFFF00DDDDDDDD7FFF77DDDDDDDDDDD777DDDD}
      OnClick = btn_imprimirClick
    end
    object SpeedButton2: TSpeedButton
      Left = 237
      Top = 2
      Width = 108
      Height = 41
      AllowAllUp = True
      Caption = '&Fechar'
      Flat = True
      Glyph.Data = {
        
        EE33337FFFFFFFFFFF7F33EEEEEEEEEEEE333377777777777773}
      NumGlyphs = 2
      OnClick = SpeedButton2Click
    end
  end
  object cb_cobrador: TRxDBLookupCombo
    Left = 7
    Top = 119
    Width = 329
    Height = 21
    DropDownCount = 8
    LookupField = 'nu_usu'
    LookupDisplay = 'Nome'
    LookupDisplayIndex = 5
    LookupSource = dts_usuarios
    TabOrder = 2
  end
  object lk_cliente: TRxDBLookupCombo
    Left = 6
    Top = 77
    Width = 331
    Height = 21
    DropDownCount = 20
    LookupField = 'pk_cliente'
    LookupDisplay = 'nome'
    LookupDisplayIndex = 5
    LookupSource = dts_clientes
    TabOrder = 3
  end
  object GroupBox1: TGroupBox
    Left = 9
    Top = 142
    Width = 167
    Height = 101
    Caption = 'Faixas de Atraso'
    TabOrder = 4
    object Label2: TLabel
      Left = 6
      Top = 24
      Width = 26
      Height = 13
      Caption = '1. De'
    end
    object Label4: TLabel
      Left = 81
      Top = 24
      Width = 15
      Height = 13
      Caption = 'at'#233
    end
    object Label5: TLabel
      Left = 6
      Top = 50
      Width = 26
      Height = 13
      Caption = '2. De'
    end
    object Label6: TLabel
      Left = 81
      Top = 50
      Width = 15
      Height = 13
      Caption = 'at'#233
    end
    object Label7: TLabel
      Left = 6
      Top = 76
      Width = 26
      Height = 13
      Caption = '3. De'
    end
    object Label9: TLabel
      Left = 81
      Top = 76
      Width = 15
      Height = 13
      Caption = 'at'#233
    end
    object fx1_de: TEdit
      Left = 37
      Top = 20
      Width = 41
      Height = 21
      TabOrder = 0
      Text = '1'
    end
    object fx1_ate: TEdit
      Left = 102
      Top = 19
      Width = 41
      Height = 21
      TabOrder = 1
      Text = '120'
    end
    object fx2_de: TEdit
      Left = 37
      Top = 46
      Width = 41
      Height = 21
      TabOrder = 2
      Text = '121'
    end
    object fx2_ate: TEdit
      Left = 102
      Top = 45
      Width = 41
      Height = 21
      TabOrder = 3
      Text = '354'
    end
    object fx3_de: TEdit
      Left = 37
      Top = 72
      Width = 41
      Height = 21
      TabOrder = 4
      Text = '355'
    end
    object fx3_ate: TEdit
      Left = 102
      Top = 71
      Width = 41
      Height = 21
      TabOrder = 5
      Text = '1800'
    end
  end
  object edt_tipo: TEdit
    Left = 82
    Top = 247
    Width = 47
    Height = 21
    TabOrder = 5
  end
  object cbx_mil: TCheckBox
    Left = 152
    Top = 248
    Width = 153
    Height = 17
    Caption = 'Converte valores em mil'
    TabOrder = 6
    Visible = False
  end
  object Dts_cliente: TDataSource
    Left = 416
    Top = 8
  end
  object DataSource1: TDataSource
    Left = 416
    Top = 8
  end
  object dts_clientes: TDataSource
    DataSet = tb_clientes
    Left = 416
    Top = 8
  end
  object usuarios: TADOTable
    Connection = DM.DB
    CursorType = ctStatic
    TableName = 'usuarios'
    Left = 128
    Top = 40
  end
  object dts_usuarios: TDataSource
    DataSet = usuarios
    Left = 160
    Top = 40
  end
  object qry_rel: TADODataSet
    Connection = DM.DB
    CommandText = 'select *'#13#10'from jobs'#13#10'where job=:rel'
    Parameters = <
      item
        Name = 'rel'
        Attributes = [paSigned, paNullable]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    Left = 64
    Top = 40
    object qry_reljob: TIntegerField
      FieldName = 'job'
    end
    object qry_relnome_campo: TStringField
      FieldName = 'nome_campo'
      Size = 50
    end
    object qry_relvalor: TBCDField
      FieldName = 'valor'
      Precision = 18
      Size = 2
    end
    object qry_reldescricao: TStringField
      FieldName = 'descricao'
      Size = 70
    end
    object qry_relcategoria: TStringField
      FieldName = 'categoria'
      FixedChar = True
      Size = 1
    end
    object qry_relchave: TIntegerField
      FieldName = 'chave'
    end
    object qry_reltag_1: TStringField
      FieldName = 'tag_1'
      Size = 50
    end
    object qry_reltag_2: TStringField
      FieldName = 'tag_2'
      Size = 50
    end
    object qry_reltag_3: TStringField
      FieldName = 'tag_3'
      Size = 50
    end
    object qry_reltag_4: TStringField
      FieldName = 'tag_4'
      Size = 50
    end
    object qry_reltag_5: TStringField
      FieldName = 'tag_5'
      Size = 50
    end
  end
  object qry_aux: TADOQuery
    Connection = DM.DB
    Parameters = <>
    Left = 96
    Top = 40
  end
  object ADOCommand1: TADOCommand
    Connection = DM.DB
    Parameters = <>
    Left = 240
    Top = 48
  end
  object DataSource2: TDataSource
    DataSet = qry_aux
    Left = 360
    Top = 66
  end
  object tb_Ocorrencias: TADOTable
    Connection = DM.DB
    TableName = 'ocorrencias'
    Left = 200
    Top = 40
    object tb_Ocorrenciasnome: TStringField
      FieldName = 'nome'
      Size = 30
    end
    object tb_Ocorrenciaspk_ocorr: TIntegerField
      FieldName = 'pk_ocorr'
    end
    object tb_Ocorrenciastipo: TStringField
      FieldName = 'tipo'
      FixedChar = True
      Size = 1
    end
  end
  object tb_titulos: TADOQuery
    Connection = DM.DB
    CursorType = ctStatic
    Parameters = <
      item
        Name = 'job'
        Attributes = [paSigned, paNullable]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'select job,'
      'tag_1 as tipo,'
      'categoria as motivo_dialogo, '
      'dec_2 as qtd_dev,'
      'dec_3 as fx1_qtd_tit,'
      'dec_4 as fx1_valor,'
      'dec_6 as fx2_qtd_tit,'
      'dec_7 as fx2_valor,'
      'dec_9 as fx3_qtd_tit,'
      'dec_10 as fx3_valor,'
      'chave as pk_devedor,'
      'nome_campo,'
      'valor,'
      'descricao'
      ''
      'from jobs where job=:job')
    Left = 312
    Top = 200
    object tb_titulosjob: TIntegerField
      FieldName = 'job'
    end
    object tb_titulosnome_campo: TWideStringField
      FieldName = 'nome_campo'
      Size = 50
    end
    object tb_titulosvalor: TBCDField
      FieldName = 'valor'
      Precision = 18
      Size = 2
    end
    object tb_titulosdescricao: TWideStringField
      FieldName = 'descricao'
      Size = 70
    end
    object tb_titulostipo: TWideStringField
      FieldName = 'tipo'
      Size = 50
    end
    object tb_titulosmotivo_dialogo: TStringField
      FieldName = 'motivo_dialogo'
      Size = 2
    end
    object tb_titulosfx1_qtd_tit: TBCDField
      FieldName = 'fx1_qtd_tit'
      Precision = 18
      Size = 2
    end
    object tb_titulosfx1_valor: TBCDField
      FieldName = 'fx1_valor'
      Precision = 18
      Size = 2
    end
    object tb_titulosfx2_qtd_tit: TBCDField
      FieldName = 'fx2_qtd_tit'
      Precision = 18
      Size = 2
    end
    object tb_titulosfx2_valor: TBCDField
      FieldName = 'fx2_valor'
      Precision = 18
      Size = 2
    end
    object tb_titulosfx3_qtd_tit: TBCDField
      FieldName = 'fx3_qtd_tit'
      Precision = 18
      Size = 2
    end
    object tb_titulosfx3_valor: TBCDField
      FieldName = 'fx3_valor'
      Precision = 18
      Size = 2
    end
    object tb_titulospk_devedor: TIntegerField
      FieldName = 'pk_devedor'
    end
    object tb_titulosqtd_dev: TBCDField
      FieldName = 'qtd_dev'
      Precision = 18
      Size = 2
    end
  end
  object tb_clientes: TADOQuery
    Connection = DM.DB
    Parameters = <>
    SQL.Strings = (
      'select * from clientes where inativado is null order by nome')
    Left = 336
    Top = 16
  end
end
